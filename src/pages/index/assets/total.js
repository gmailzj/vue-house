...
let $na = function(fname) {
  let objs = document.getElementsByName(fname);
  if (objs) {
    return objs[0];
  }
  return false;
...
function ShowLilv(month, lt) {
  let indexNumSd = getArrayIndexFromYear(month, 1); // 商贷
  let indexNumGj = getArrayIndexFromYear(month, 2); // 公积金
...
function showRightDiv(fmobj) {
  ...
  let a = window.open('', 'calc_win', 'status=yes,scrollbars=yes,resizable=yes,width=550,height=500,left=0,top=0')// 790*520
  if (fmobj.name == "calc1") {
...
function getlilv(lilv_class, type, years) {
  let indexNumSd = getArrayIndexFromYear(years, 1); // 商贷
  let indexNumGj = getArrayIndexFromYear(years, 2); // 公积金
...
function ext_total(fmobj) {
  ...
  let years = fmobj.years.value;
  let month = fmobj.years.value * 12;
  ...
  if (fmobj.type.value == 3) {
    ...
    // 月还款
    let lilv_sd = $('sdlv').value / 100;// 得到商贷利率
    let lilv_gjj = $('gjlv').value / 100;// 得到公积金利率
    ...
    // --  商业贷款、公积金贷款
    let lilv = $('singlelv').value / 100;// 得到利率
    if (fmobj.jisuan_radio[0].checked == true) {
      ...
      // 房款总额
      let fangkuan_total = fmobj.price.value * fmobj.sqm.value;
      ...
      fmobj.daikuan_total2.value = daikuan_total;
      // 首期付款
      let money_first = fangkuan_total - daikuan_total;
...
function chanage_type_mm() {
  if ($("dengeben2").checked) { $("type1_mm1").innerHTML = '&nbsp;&nbsp;月均金额：<textarea name="month_money1" rows="5" ></textarea>'; } else { $("type1_mm1").innerHTML = '&nbsp;&nbsp;月均还款：<input name="month_money1" type="text" class="guestbook02" />元'; }
}
...
function isIdCardNo(sNo) {
  ...
  } else if (!isInt(sNo)) { return false; }
...
function isValidDate(iY, iM, iD) {
  ...
  if (iY != undefined && !isNaN(iY) && iY >= 0 && iY <= 9999 && iM != undefined && !isNaN(iM) && iM >= 1 && iM <= 12 && iD != undefined && !isNaN(iD) && iD >= 1 && iD <= 31) {
    ...
    if (iM == 2 && (isLeapYear(iY) && iD > 29 || !isLeapYear(iY) && iD > 28) || iD == 31 && (iM < 7 && iM % 2 == 0 || iM > 7 && iM % 2 == 1)) { return false; }
    return true;
  } return false;
}
...
function Format(num, dotLen) {
  ...
  let num1 = 0
  // change by liyugen
  if (typeof dotLen === "undefined") { dot = 2 } else { dot = dotLen }
  if (isNaN(parseFloat(num))) { return 0 }
  num1 = parseFloat(num)
  let n1 = Math.pow(10, dot)
  if (n1 == 0) { var iValue = Math.round(num1) } else { var iValue = Math.round(num1 * n1) / n1 }
  let sValue = iValue.toString();
  if (sValue.indexOf(".") == -1) {
    sValue += ".00";
  } else if (sValue.indexOf(".") == sValue.length - 1) {
    sValue += "00";
...
function c_id_card(obj) {
  let id_card = obj.id_card.value;// 身份证
  let age = 0;
  age_qx.innerText = '';
  if (id_card.length > 0) {
...
function gjjloan1(obj) {
  ...
  let jtysr;// 家庭月收入
  let r;// 月还款
  let gjjdka;// 住房公积金贷款额度a
  let gjjdkb;// 住房公积金贷款额度b
  let gjjdkc;// 住房公积金贷款额度c
  ...
  if (poyjce.length > 0 && !isnumeric(poyjce)) { alert("配偶月缴存额录入不正确");return; }
  ...
  */
  if (obj.xz[0].checked == true) { fwxz = 0.9; } else { fwxz = 0.8; }

  if (obj.xy[0].checked == true) { xy = 1.3; } else if (obj.xy[1].checked == true) { xy = 1.15; } else { xy = 1; }
...
function gjjloan2(obj) {
  ...
  */
  ...
  if (obj.xy[0].checked == true) { xy = 1.15; } else if (obj.xy[1].checked == true) { xy = 1.3; } else { xy = 1; }
  ...
  if (hkfs == 1) {
    ...
    if (dknx >= 1 && dknx <= 5) { ylv_new = l1_5 / 12; } else { ylv_new = l6_30 / 12; }


    let ncm = parseFloat(ylv_new) + 1;// n次幂

    let dknx_new = dknx * 12;


    total_ncm = Math.pow(ncm, dknx_new)

    obj.ze22.value = Math.round(((dked * 10000 * ylv_new * total_ncm) / (total_ncm - 1)) * 100) / 100;
    let pp = Math.round(((dked * 10000 * ylv_new * total_ncm) / (total_ncm - 1)) * 100) / 100;
  ...
  if (hkfs == 2) {
    ...
    bxhj = syhk;
    for (let count = 2;count <= dkys; ++count) {
...
function gjjloan3(obj) {
  ...
  let ll; // 月利率
  if (obj.lx8[1].checked) { ll = Math.round(1000000 * l6_30 / 12) / 1000000; } else { ll = Math.round(1000000 * l1_5 / 12) / 1000000; }
  ...
  let first_lx = Math.round(dkye * ll * 100) / 100 ;
  if (first_lx > gdhke) { alert('固定还款额必须大于首月利息 ' + first_lx);obj.lx9.focus();obj.lx9.select();return; }
  let yjqs = 0;// Math.ceil(dkye/gdhke);
  var sxhky = 0;
  for (let i = 1;i < syqs;i++) {
    // 需要还款月+1
    sxhky += 1;
    // 计算一个月的利息
...
function chk01() {
  if (parseFloat(document.myform.rg01.value) < 4.7) { alert("--您确定是" + parseFloat(document.myform.rg01.value) + "万元?--" + "\n\n" + "那么您目前尚不具备购房能力，" + "\n\n" + "建议积攒积蓄或能筹集更多的资金。") }
  if (parseFloat(document.myform.rg01.value) > 10000) { alert("您确定拥有超过一亿元的购房资金？"); }
}
function chk02() {
...
function chk03() {
  if (document.myform.rg01.value == "") { alert("请填写现可用于购房的资金") } else
  if (document.myform.rg02.value == "") { alert("请填写现家庭月收入") } else
  if (document.myform.rg03.value == "") { alert("请填写预计家庭每月可用于购房支出") } else
  if (document.myform.rg06.value == "") { alert("请填写您计划购买房屋的面积") } else { chk04() }
}
function chk03_coop() {
  if (document.myform.rg01.value == "") { alert("请填写现可用于购房的资金") } else
  if (document.myform.rg02.value == "") { alert("请填写现家庭月收入") } else
  if (document.myform.rg03.value == "") { alert("请填写预计家庭每月可用于购房支出") } else
  if (document.myform.rg06.value == "") { alert("请填写您计划购买房屋的面积") } else { chk04_coop() }
}
function chk04_coop() {
  // 您可购买的房屋总价=（家庭月收入-家庭月固定支出）×( (（1＋月利率）＾还款月数)－1  )÷［月利率×（1＋月利率）＾还款月数］+持有资金
  let month = parseInt(document.myform.rg04.options[document.myform.rg04.selectedIndex].value);
  let year = parseInt(month / 12);
  let lilu = 0.00576;
  if (year > 5) { lilu = 0.00594; }
  ...
  let d1 = js01 - js02;
  let d2 = Math.pow(1 + lilu, month) - 1;
  let d3 = lilu * Math.pow(1 + lilu, month)
...
function show(nid) {
  for (i = 1;i < 4;i++) {
    let oid = "re_inner_" + i;
    if (i == nid) { document.getElementById(oid).style.display = ""; } else { document.getElementById(oid).style.display = "none"; }
  }
}
